...
- **Default patterns live only in `action.yml`**: `src/__tests__/tally.test.ts` parses that file and asserts real paths classify correctly. Don't copy the defaults into code or tests -- the copy would drift.
- **A loose schema should not leak an index signature into its type**: `z.infer` of a `loose()` object carries `{ [k: string]: unknown }`, which makes every typo readable as `unknown`. `run.ts` wraps the inference in type-fest's `OmitIndexSignature`, so parsing stays tolerant of a new cloc field while the type stays closed. type-fest is a devDependency and ships only declarations, so it costs nothing at runtime.
- **es-toolkit before a hand-rolled fold**: `mapValues`, `sumBy` and `zipObject` are all in use; `zipObject` also types the record it builds, which a `Object.fromEntries` needs a cast for.
