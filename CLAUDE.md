...
| `npm run cloc:check`| Compare the cloc pin against upstream's latest |
...
- **`cloc`'s npm package is not versioned like `cloc`**: the registry's `cloc` package numbers its releases independently of the tool it bundles — `cloc@2.06` ships cloc **1.86**, which reads a rename as a whole file added plus a whole file deleted. `src/cloc/download.ts` pins upstream's release script by URL and sha256 instead; keep it that way. `cloc-version.yml` opens a pull request when upstream moves on, rewriting both constants together and rebuilding `dist/`. cloc has no programmatic API and never will (it is Perl), and the JS counters that do have one — `sloc` and friends — neither diff two revisions nor survive a `//` inside a string, so a subprocess is the design, not a stopgap.
